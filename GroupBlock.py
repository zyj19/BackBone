...
class My_gcn(Layer):
    ...
    def __init__(self,
                 filters,
                 # kernel_initializer=tf.keras.initializer,
        ...
        self.kernel_regularizer = tf.initializers.get(kernel_regularizer)
        self.kernel_constraint = tf.initializers.get(kernel_constraint)
        self.activation = Activation('relu')
        super(My_gcn, self).__init__(**kwargs)

    def build(self, input_shape):
        F = input_shape[0][-1]
        self.kernel = self.add_weight(
            shape=(F, self.filters),
        ...
        self.built = True
        super(My_gcn, self).build(input_shape)

    def call(self, inputs, **kwargs):
        x, a = inputs
        output = K.dot(x, self.kernel)
        output = ops.modal_dot(a, output)
        # output = K.dot(x, output)
