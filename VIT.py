...
class AddPositionEmbs(Layer):
    def __init__(self, image_shape, patch_size, pe_initializer='zeros', pe_regularizer=None, pe_constraint=None,
                 **kwargs):
    ...
    def call(self, inputs, **kwargs):
        ...
        img_token_pe = tf.compat.v1.image.resize_bicubic(img_token_pe, (
        self.image_shape[0] // self.patch_size, self.image_shape[1] // self.patch_size), align_corners=False)
...
class Attention(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        # -----------------------------------------------#
        inputs = Reshape((-1, 3, self.num_heads, self.projection_dim))(inputs)
        # -----------------------------------------------#
        ...
        # -----------------------------------------------#
        output = Reshape((value.shape[1], -1))(value)
        return output
...
def VisionTransformer(input_shape=None, patch_size=16, num_layers=12, num_features=768, num_heads=12,
                      mlp_dim=3072,
