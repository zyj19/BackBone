...
def resblock_body(x, num_filters, num_blocks, expansion=0.5, shortcut=True, last=False, name=""):
    ...
    return C3(x, num_filters, num_blocks, shortcut=shortcut, expansion=expansion,
...
def creat_model(phi='l', classes=2):
    ...
    x1, x2, x3 = darknet_body(inputs, base_channels, base_depth=base_depth)

    out = GlobalAveragePooling2D()(x3)
    out = SiLU()(out)
    out = Dropout(.3)(out)
