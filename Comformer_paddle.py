...
class ConvBlock(nn.Layer):
    ...
    def __init__(self, inplanes, outplanes, stride=1, res_conv=False, act_layer=nn.ReLU, groups=1,
        ...
        if res_conv:
            self.residual_conv = nn.Conv2D(inplanes, outplanes, kernel_size=1, stride=stride, padding=0,
                                           bias_attr=False)
...
class Med_ConvBlock(nn.Layer):
    ...
    def __init__(self, inplanes, act_layer=nn.ReLU, groups=1, norm_layer=partial(nn.BatchNorm2D, epsilon=1e-6),
        ...
        self.conv2 = nn.Conv2D(med_planes, med_planes, kernel_size=3, stride=1, groups=groups, padding=1,
                               bias_attr=False)
        self.bn2 = norm_layer(med_planes)
        self.act2 = act_layer()
